    ...
    const fetchRoute = async () => {
        ...
        try {
            ...
            if (response.data.routes && response.data.routes.length) {
                ...
                console.log('Route and steps fetched. Steps count:', steps.length);
                const fetchedRouteData = { coords, steps };
                setRouteData(fetchedRouteData);

                return fetchedRouteData;
            } else {
                console.error('No route found. Full response:', response.data);
                const status = response.data.status;
                const errorMessage = response.data.error_message || 'Please check API key billing/permissions and try again.';
                Alert.alert('No route found', `${status || 'UNKNOWN_ERROR'}: ${errorMessage}`);
                return null;
            }
        } catch (error: any) {
            console.error('Error fetching route:', error?.message, error?.response?.data);
            const apiError = error?.response?.data?.error_message || error?.response?.data || error?.message || 'Unknown error';
            Alert.alert('Error', `Failed to fetch route. ${String(apiError)}`);
            return null;
        }
    };
